...
const char ssid[] = "xda-2.4";           // Change to your Router SSID
const char password[] = "lspci|grep@Wi"; // Change to your Router Password
...
static char history[HISTORY_LENGTH][BUF_LENGTH];
...
const char *getHistory(int index)
{
  if (index >= history_count)
    return "";
  int realIndex = (history_index + index + 2 * history_count) % history_count;
  return history[realIndex];
}
void pushHistory(const char *str)
{
  // if it's equal to last saved command ignore it
  if (history_count > 0 && strcmp(str, getHistory(-1)) == 0)
    return;
  strncpy(history[history_index], str, BUF_LENGTH - 1);
  history[history_index][BUF_LENGTH - 1] = '\0'; // Null-terminate
  history_index = (history_index + 1) % HISTORY_LENGTH;
  if (history_count < HISTORY_LENGTH)
    history_count++;
}
void adjust_history(int count = 1)
{
  if (history_count > 0)
  {
    history_cursor = (history_cursor - count + history_count) % history_count;
  }
}
int setandshowHistory(WiFiClient &tcpclient, char *buf)
{
  if (history_count == 0)
    return 0;
  const char *command = getHistory(history_cursor);
  if (strlen(command) == 0)
  {
    tcpclient.printf("No command at this index(%d).\n", history_cursor - 1);
    return 0;
  }
  else
  {
    tcpclient.printf("\r                         \r$ %s", command);
  }
  strncpy(buf, command, BUF_LENGTH - 1);
  return strlen(buf);
}
void print_list()
{
  Serial.println("HistotyList: ");
...
static void exec(char *cmdline)
{
  ...
  int rssi;
  int mark_rssi = -100;
  bool should_be_saved = true; // flag to save command to history
  ...
  {
    tcpclient.write("Echo mode is changed to ");
  ...
  else if (strcmp_P(command, PSTR("x")) == 0)
  {
    ...
    jammingmode = 0;
    recordingmode = 0;
    tcpclient.write("All modes stopped.");
    // needed for ESP8266
    yield();
  ...
  {
    tcpclient.write("Rebooting the chip...");
    ESP.wdtFeed();
    yield();
    delay(200);
    ESP.restart();
  }
  else if (strcmp_P(command, PSTR("exit")) == 0)
  {
    tcpclient.stop();
    yield();
  }
  ...
  if (should_be_saved)
  {
    pushHistory(command); // save command to history
  }
  tcpclient.write("\r\n$ "); // output CRLF and prompt
}
...
void setup()
{
  ...
  while (Serial)
    ;
  digitalWrite(LED_BUILTIN, LOW);
  Serial.println("CC1101 terminal tool started...");
  ...
  */
  ...
  if (!MDNS.begin("cc1101-tools"))
  { // results in cc1101.local
    Serial.println("Error setting up MDNS responder!");
  }
  else
  {
    Serial.println("mDNS responder started as cc1101.local");
  }
...
void loop()
{
  ...
  static int length = 0;
  char destination[8];
  int escState = 0; // state of escape sequence
      ...
      // read data from the connected tcpclient
      {
        ...
        else
        {
          ...
          { // normal state, not in escape sequence
            ...
            if (data == 0x1B)
            {
              escState = 1; // start escape sequence
              ESP.wdtFeed();
              yield();
            }
          ...
          { // escape sequence state
            if (data == '[')
            {
              escState = 2; // start of escape sequence
              ESP.wdtFeed();
              yield();
            }
            else
            {
              escState = 0; // not in escape sequence
              ESP.wdtFeed();
              yield();
            }
          }
          ...
          { // in escape sequence
            ...
            { // up arrow
              if (length > 0)
              { // if current command is not the same as the last one
                buffer[length] = '\0';
                pushHistory(buffer); // save current command to history
                adjust_history(1);
              }
              adjust_history(1); // move to previous command in history
              length = setandshowHistory(tcpclient, buffer); // show previous command from history??
              ESP.wdtFeed();
              yield();
            }
            else if (data == 'B')
            {
              adjust_history(-1);                            // move to next command in history
              length = setandshowHistory(tcpclient, buffer); // show previous command from history??
              ESP.wdtFeed();
              yield();
            ...
            {               // right or left arrow, do nothing
              escState = 0; // reset state
            }
            else
            {
              escState = 0; // reset state for any other character
            }
          }
        };
